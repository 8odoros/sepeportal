...
@Service
@RepositoryEventHandler(SpPtlCompIeAnnBook.class)
public class CompanyIEAnnounBookEventHandler {

    @Autowired
    SpPtlCompanyUserPrivilagesRepository companyUserPrivilagesRepository;

    private static final Logger LOGGER = LoggerFactory.getLogger(CompanyIEAnnounBookEventHandler.class);

    @HandleBeforeSave
    public void handleBeforeSave(SpPtlCompIeAnnBook compIeAnnBook) throws Exception {
        LOGGER.debug("Handling SpPtlCompIeAnnBook for: User ID " + CurrentUserDetailsService.getCurrentUser().getUsername());
        ...
        if (userDTO.getRole() == Constants.USER_TYPE.COMPANY_USER.getCode()) {
            ...
            if(null != priv.getPrivilages() && list.contains(String.valueOf(Constants.COMPANY_SERVICE.BOOK_SUGGESTIONS.getCode()))) {
                ...
                compIeAnnBook.setCompanyId(companyId);
            }
            else {
            ...
            compIeAnnBook.setCompanyId(currentUser.getUserId());
        }
    }

    @HandleBeforeCreate
    public void handleBeforeCreates(SpPtlCompIeAnnBook compIeAnnBook) throws Exception {
        LOGGER.debug("Handling SpPtlCompIeAnnBook for: User ID " + CurrentUserDetailsService.getCurrentUser().getUsername());
        SpringUserDetails currentUser = CurrentUserDetailsService.getCurrentUser();
