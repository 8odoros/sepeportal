 ...
 */
// http://localhost:7001/compExtraInfo
@Transactional(propagation = Propagation.REQUIRES_NEW)
@RepositoryRestResource(collectionResourceRel = "compExtraInfo", path = "/compExtraInfo")
public interface SpPtlCompExtraInfoRepo extends PagingAndSortingRepository<SpPtlCompExtraInfo, Long> {
    @Query(
            /*value = "SELECT o FROM SpPtlCompExtraInfo o "
                    + "WHERE  o.companyId =?#{principal.company.id}"
                    + " AND "
                    + " UPPER(?#{principal.branchIds}) LIKE CONCAT('%,' , o.branch1Id , ',%')"*/
            value = "SELECT DISTINCT a FROM SpPtlCompExtraInfo a, TCompanyUserPrivilages p, TUser u "
    ...
    @Nullable
    public Page<SpPtlCompExtraInfo> findAll(Pageable pageable);
    ...
    @Nullable
    public SpPtlCompExtraInfo findOne(@Param("id") Long id);

    public SpPtlCompExtraInfo save(SpPtlCompExtraInfo entity);

    @Modifying
    @Transactional
    @Query(
            value = "DELETE FROM SpPtlCompExtraInfo o "
                    + "WHERE  o.companyId =?#{principal.company.id} AND o.id =:#{#id}"
                    + " AND o.subStatus < 2"
                    //+ " AND "
                    //+ " UPPER(?#{principal.branchIds}) LIKE CONCAT('%,' , o.branch1Id , ',%')"
